...
Puppet::Type.type(:package).provide :pkgng, :parent => Puppet::Provider::Package do
  ...
  def self.get_latest_version(origin,versionlist)
    if latest_version = versionlist.lines.find { |l| l =~ /^#{origin} / }
  ...
  def self.instances
	...
	versionlist = self.get_version_list
      info.lines.each do |line|
        ...
        latest_version  = get_latest_version(origin,versionlist) || version
